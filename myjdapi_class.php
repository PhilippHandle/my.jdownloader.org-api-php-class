 ...
 *
 * @author Anatoliy Kultenko "tofik", Dominik Schwarzbauer "R3DST0RM"
 ...
 */
 ...
 class MYJDAPI
 {
     ...
     // @returns PackageId
     public function addLinks( $links, $package_name = null, $path = "/media/HDD2/") {
         ...
         $params = '\"dataURLs\":[], \"links\":\"'.$links.'\", \"packageName\":\"'.$package_name.'\", \"destinationFolder\":\"'.$path.'\", \"extractPassword\":\"\", \"downloadPassword\":\"\", \"overwritePackagizerRules\":true, \"autoExtract\":true, \"autostart\":false, \"deepDecrypt\":false';
     ...
     public function getDownloadFolderHistorySelectionBase(){
         if( !is_array( $this->devices )){
             $this->enumerateDevices();
         }

         $res = $this->callAction( "/linkgrabberv2/getDownloadFolderHistorySelectionBase" );
     ...
     public function getPackageCount(){
         ...
         $res = $this->callAction("/linkgrabberv2/getPackageCount");
         return $res;
     }

     // Get all packages currently queued
     public function queryPackages(){
         ...
         $res = $this->callAction("/linkgrabberv2/queryPackages", $params);

         return $res;
     }

     /**
      * Moves a specific package to the download list
      * @param $packageUUID
      * @return bool|mixed|string
      */
     public function moveToDownloadList($packageUUID){
         if(!is_array($this->devices))
             $this->enumerateDevices();

         $params = '"[]", "['.$packageUUID.']"';
         $res = $this->callAction("/linkgrabberv2/moveToDownloadlist", $params, true);
         return $res;
     }

     public function setDownloadPath($downloadPath, $packageId){
         if(!is_array($this->devices)){
             $this->enumerateDevices();
         }

         $params = '"\"'.$downloadPath.'\"","['.$packageId.']"';
         $res = $this->callSpecificAction("/linkgrabberv2/setDownloadDirectory", $params);
     ...
     // return: result from server or false
     public function callAction( $action, $params = false, $passParams = false) {
         if( !is_array( $this -> devices)) {
             $this -> enumerateDevices();
         }
     ...
     public function callSpecificAction( $action, $params = false) {
         if( !is_array( $this -> devices)) {
             $this -> enumerateDevices();
         }
